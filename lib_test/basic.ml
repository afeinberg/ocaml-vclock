...
let test_maybe_compare _ =
  ...
  let b = V.create "b" (VectorClock.incremented 0 1L VectorClock.empty) in
  let c = V.create "c" (VectorClock.incremented 1 1L VectorClock.empty) in
  let d = V.create "d" (VectorClock.incremented 0 2L VectorClock.empty) in 
  assert_equiv a a ;
  assert_before a b ;
  assert_after d b ;
...
let test_conflict_resolver _ =
  let a = VectorClock.incremented 0 1L VectorClock.empty in
  let b = VectorClock.incremented 1 2L a in
